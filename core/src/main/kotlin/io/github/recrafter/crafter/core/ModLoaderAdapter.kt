...
abstract class ModLoaderAdapter {
    ...
    abstract fun configurePlugin(
        ...
        runDirectory: File,
        accessConfig: File,
        isRunConfigurationsDisabled: Boolean = true,
    )
    ...
    fun configureInternal(
        ...
        craftModConfigTask: TaskProvider<out Task>,
    ) = with(project) {
        group = mod.namespace
        base {
            archivesName = mod.id
        ...
        val accessConfig = craftedResourcesDirectory.resolve(mod.accessConfigPath).ensureFileExists().apply {
        ...
        tasks {
            ...
            if (isDataPackConfigRequired()) {
                val craftDataPackConfigTask = registerTask<CraftDataPackConfigTask> {
                    this.mod.set(mod)
                    minFormat = mod.minMinecraftVersion.getDataPackFormat(project)
                    maxFormat = mod.maxMinecraftVersion.getDataPackFormat(project)
