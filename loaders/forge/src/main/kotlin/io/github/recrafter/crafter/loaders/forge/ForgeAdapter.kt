...
object ForgeAdapter : ModLoaderAdapter() {
    ...
    override fun configurePlugin(
        mod: Mod, project: Project, runDirectory: File, accessorConfig: File?
    ) =
        with(project) {
            minecraftForge {
                mappings("official", mod.minecraftVersion.asString())
                accessorConfig?.let { atFile ->
                    setAccessTransformer(true)
                    accessTransformer.setFrom(atFile)
                }
            ...
            tasks {
                jar {
                    manifest {
                        attributes["MixinConfigs"] = mod.mixinsConfigPath
                    }
                }
            }
            groupLoaderTasks(
